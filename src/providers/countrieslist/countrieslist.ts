...
/*
  Generated class for the CountrieslistProvider provider.

  See https://angular.io/guide/dependency-injection for more info on providers
  and Angular DI.
*/
@Injectable()
export class CountrieslistProvider {

  constructor(public http: HttpClient) {  
    console.log('Hello CountriesListProvider Provider');
  }

  getCountryNames(searchTerm) {
    let countryCodeList = this.getCountryCodes();
    return countryCodeList.filter((country) => {
      return country.name.toLowerCase().indexOf(searchTerm.toLowerCase()) > -1;
    }).map((country) => country.name);
  }

  getCountryCoodes(searchTerm) {
    let countryCodeList = this.getCountryCodes();
    return countryCodeList.filter((country) => {
  ...
  public getCountryCodes() {
    let countryCodeList = [{ "name": "Afghanistan", "dial_code": "+93", "code": "AF" }, {
      "name": "Aland Islands",
      "dial_code": "+358",
      "code": "AX"
